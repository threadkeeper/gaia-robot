...
| Q3 | `DataLakeIndex` partition key / source | **Partition by `/entity`, identical to `GaiaDataLake`**, fed from `GaiaDataLake`. Keep an optional `source` field for future DataLake sources. |
...
| Q6 | Which containers get the writer | **`GaiaKB`, `GaiaDataLake`, `GaiaDiary`.** Exclude `GaiaConnections` (ledger) and `GaiaWebSearchHistory` (audit log). |
